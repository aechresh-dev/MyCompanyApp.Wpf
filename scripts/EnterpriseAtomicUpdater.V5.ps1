...
function Install-Package {
    ...
    try {
        ...
        Save-State -CurrentPhase 'Completed' -RecoveryNeeded $false -RecoveryReason ''
        Clear-State
...
function Invoke-Rollback {
    ...
    if ([string]::IsNullOrWhiteSpace($previous)) {
        throw "No previous version recorded for rollback"
    }
    ...
    if ($state.RecoveryNeeded -eq $true) {
        switch ([string]$state.CurrentPhase) {
            'DeployStarted' {
                ...
                if (Test-Path $PreviousLinkPath) {
                    Invoke-Rollback
                }
                else {
                    Write-Warn "No previous link file found."
                }
            }
            'HealthCheckStarted' {
